...
<Steps>
1. Create a script and place it somewhere. For example `/tmp/rmpc/notify`. Below is an example of such script.
Edit it to your needs or create a new one.

    ```bash title=/tmp/rmpc/notify
    #!/usr/bin/env sh
...
2.  In your rmpc's `config.ron` specify [on_song_change](/rmpc/configuration#on_song_change) property
    and point it at location of the script from step 1.
        ...
        (
            ...
            on_song_change: ["/tmp/rmpc/notify"],
        ...
