...
The `StateV2` property has additional configuration options. You can set what text appears for each state by specifying
`Status(StateV2(playing_label: "<your label>", paused_label: "<your label>", stopped_label: "<your label>"))`.
If you omit any of the values, the default is used, so you can for example do just: `Status(StateV2(playing_label: "Playing"))`
or `Status(StateV2())`.
