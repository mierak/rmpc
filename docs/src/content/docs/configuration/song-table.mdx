...
<ConfigValue name="kind" type="other" customText="Group(<property>)" link="#property" />A special kind of property that
groups multiple properties together. If any of the properties in the group results in a `None` value, the whole group is
considered `None` and the default value is used.
...
If the artist or tilte of the song is missing is missing in its metadata, the whole group will be skipped, including the `Text`
property and the song's filename will be displayed instead.
