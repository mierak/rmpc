...
Can display values from currently playing song metadata, just like in the [song
table](/rmpc/configuration/song-table/#property), info about the player status (volume, playing or stopped, current
bitrate, ...) or a predefined widget.
