...
use log::info;
use rmpc_mpd::{
    client::{Client, MIN_SUPPORTED_VERSION},
...
fn main() -> Result<()> {
    ...
    match args.command {
        ...
        None => {
            ...
            if client.version < MIN_SUPPORTED_VERSION {
                status_warn!(
                    "MPD version '{}' is lower than supported. Minimum supported protocol version is '{MIN_SUPPORTED_VERSION}'. Some features may work incorrectly.",
                    client.version
                );
            }
